...
def run_remix(
    ...
    # 1. Determine source/target tempo.
    if not source_bpm:
        source_bpm = estimate_tempo(vocal_wav, sr)
        logger.info("Estimated source vocal tempo: %.1f BPM", source_bpm)
    if not source_bpm:
        raise ValueError(
    ...
    # 3. Build the instrumental prompt (instrumental + style tags + target BPM).
    style_fragment = ""
    if references:
    ...
    # 4. Generate the instrumental, matched to the stretched vocal's length.
